...
HeroManager = List.new() -- ”¢–€±Ì
MonsterManager = List.new() -- π÷ŒÔ±Ì

local function solveCollision(object1, object2) -- ≈ˆ◊≤ºÏ≤‚
...
local function isOutOfBound(object) -- ∏¸–¬”¢–€/π÷ŒÔ≤ª≥¨≥ˆªÓ∂Ø∑∂Œß
...
function collisionDetect(dt)
    ...
    for val = HeroManager.last, HeroManager.first, -1 do
        local sprite = HeroManager[val]
        if sprite._isalive == true then --  «∑ÒÀ¿Õˆ
            collision(sprite) -- ”Î”¢–€/π÷ŒÔ≈ˆ◊≤
            isOutOfBound(sprite)
            sprite._effectNode:setPosition(sprite._myPos)
        else
            List.remove(HeroManager, val) -- À¿Õˆ…æ≥˝
        end
    end
